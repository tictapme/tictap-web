...
function buildSitemapIndexXml() {
  ...
  const body = SITEMAP_FILES.filter((fileName) => fs.existsSync(path.join(SRC_DIR, fileName)))
    .map((fileName) => {
      ...
      const sitemapXml = fs.readFileSync(fullPath, 'utf8');
      const lastmod = extractSitemapLastmod(sitemapXml);
...
function extractSitemapLastmod(xml) {
  const matches = [...xml.matchAll(/<lastmod>([\s\S]*?)<\/lastmod>/g)];
  let latestValue = '';
