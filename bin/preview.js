...
function rewriteContent(content, localOrigin) {
  ...
  // Avoid rewriting links already pointing to the active preview origin.
  const normalizedOrigin = escapeRegExp(localOrigin);
  rewritten = rewritten.replace(
    new RegExp(`${normalizedOrigin}${normalizedOrigin}`, 'g'),
    localOrigin,
  );

  const escapedLocalOrigin = escapeRegExp(localOrigin.replace(/\//g, '\\/'));
  rewritten = rewritten.replace(
    new RegExp(`${escapedLocalOrigin}${escapedLocalOrigin}`, 'g'),
